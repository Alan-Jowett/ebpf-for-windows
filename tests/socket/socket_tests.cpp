    ...
    uint32_t protocol)
{
    ...
    bpf_map* nexthop_interface_map = bpf_object__find_map_by_name(object, "nexthop_interface_map");
    SAFE_REQUIRE(nexthop_interface_map != nullptr);
    ...
    uint64_t nexthop_interface_luid = 0;
    result = bpf_map_lookup_elem(bpf_map__fd(nexthop_interface_map), &connection_id, &nexthop_interface_luid);
    if (result == 0) {
        printf("Nexthop interface LUID: %llu\n", nexthop_interface_luid);
        ...
        SAFE_REQUIRE(nexthop_interface_luid >= 0);
    }
    ...
    if (result == 0) {
        printf(
            "Comprehensive results - Interface: %u, Tunnel: %u, Nexthop: %llu, SubInterface: %u, ConnID: %u\n",
            comprehensive_results.interface_type,
            comprehensive_results.tunnel_type,
            comprehensive_results.nexthop_interface_luid,
        ...
        SAFE_REQUIRE(comprehensive_results.nexthop_interface_luid == nexthop_interface_luid);
        ...
        bool has_network_context = (comprehensive_results.interface_type != 0) ||
                                   (comprehensive_results.nexthop_interface_luid != 0) ||

...
\@x{ {\EXTENDS} EpochModel ,\, Naturals}%
\@x{}%
\@y{\@s{0}%
 Copyright (c) \ensuremath{eBPF} for Windows contributors
}%
\@xx{}%
\@x{}%
\@y{\@s{0}%
...
\begin{cpar}{0}{F}{F}{0}{0}{}%
 This is intentionally a very small, starter \ensuremath{TLAPS} proof: it
 shows how to turn
 an existing model into a proof obligation and discharge it with
 \ensuremath{TLAPS}.
\end{cpar}%
\vshade{5.0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
...
\end{cpar}%
\begin{cpar}{1}{F}{F}{0}{0}{}%
- We start with proving that \ensuremath{Init} implies \ensuremath{TypeOK}.
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
**************************************************************************
\end{cpar}%
\end{lcom}%
\@x{ {\THEOREM} InitImpliesTypeOK \.{\defeq}}%
\@x{\@s{8.2} {\ASSUME}}%
\@x{\@s{16.4} \.{\land} NCPUS \.{\in} Nat}%
\@x{\@s{16.4} \.{\land} MaxEpoch \.{\in} Nat \.{\land} MaxEpoch \.{\geq} 1}%
\@x{\@s{16.4} \.{\land} UsePublishedEpochForReader \.{\in} {\BOOLEAN}}%
 \@x{\@s{16.4} \.{\land} UsePublishedEpochForRetire\@s{4.15} \.{\in}
 {\BOOLEAN}}%
\@x{\@s{8.2} {\PROVE} Init \.{\implies} TypeOK}%
\@x{ {\PROOF}}%
\@x{\@s{8.2} {\BY} {\DEF} Init ,\, TypeOK ,\, CPUS ,\, ObjStates}%
\@pvspace{8.0pt}%
\@x{}\bottombar\@xx{}%
\end{document}

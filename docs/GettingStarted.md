...
##### Compile time options (deprecated)

> **Note**: JIT and Interpreter execution modes have been deprecated as of Issue #4997. Only native execution
> (using pre-compiled .sys drivers generated by bpf2c) is now supported. The `DisableJIT` and `DisableInterpreter`
> flags are now always enabled by default, and the `NativeOnlyDebug`/`NativeOnlyRelease` configurations are
> functionally equivalent to `Debug`/`Release`.
...
- `ebpfsvc.exe`: A user-mode service that verifies eBPF programs and authorizes native modules.

...
##### Compile time options (legacy)

> **Note**: Only native execution (using pre-compiled .sys drivers generated by bpf2c) is supported. The `DisableJIT`
> and `DisableInterpreter` flags are always enabled by default.
